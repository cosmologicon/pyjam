...
data = []
headstart = 0
toclaim = set(tuple(lock) for lock in locks)
lockps = []
...
for jregion, region in enumerate(regions):
	...
	def nearregion(p0, r = 3):
		return any(inregion(p) for p in math.CSround(12, r = r, center = p0))
	ldata = {}
	data.append(ldata)
	ldata["region"] = region
	...
	assert len(cps) == 1
	ldata["youpos"] = cps[0]
	ldata["youtheta"] = checkpoints[cps[0]] * 24 / math.tau
	...
	for objpos, objtype in objs.items():
		...
		r = 3 if "big" in objtype else 0.5
		objspec = {
			"pos": objpos,
			"numreq": numreq,
			"windreq": windreq,
