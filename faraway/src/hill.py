...
# Hill Surfaces are generated when the hill object is loaded, and eventually removed once the
# Surface cache reaches a large enough size. For performance reasons, hills are not completely
# drawn to their Surfaces when first loaded. The drawing function is a generator, which does a small
# amount of work before yielding. The killtime function spends the given amount of time working on
# any hill Surfaces that are not complete. For smoother gameplay, this is called every frame with
# some small amount of time. If a hill is requested to be drawn to the actual screen, and its
# Surface is not yet ready, the draw command will block while the Surface finishes.
